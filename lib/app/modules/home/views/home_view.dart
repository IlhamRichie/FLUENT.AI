...
class HomeView extends GetView<HomeController> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            Image.asset('assets/images/logo FLUENT.png', height: 28, width: 28), // Ukuran logo disesuaikan
            const SizedBox(width: 8),
            Text('FLUENT', style: TextStyle(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 26, letterSpacing: -0.5)),
          ],
        ),
        actions: [
          Stack(
            children: [
              IconButton(
                ...
                onPressed: () => Get.snackbar('Notifikasi', 'Belum ada notifikasi baru.', snackPosition: SnackPosition.TOP, margin: const EdgeInsets.all(10), borderRadius: 10),
                tooltip: 'Notifikasi',
              ),
              Positioned( // Indikator notifikasi (dummy)
                right: 10, top: 10,
                child: Container(
        ...
        return RefreshIndicator( // Tambahkan RefreshIndicator
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              _buildSectionHeader('Mulai Latihan Cepat', LucideIcons.zap, primaryColor),
              _buildQuickActionsGrid(theme),
              _buildSectionHeader('Aktivitas Terakhir', LucideIcons.history, primaryColor, onViewAll: controller.viewAllActivities),
              _buildLatestActivitiesList(theme),
              _buildSectionHeader('Rekomendasi Untukmu', LucideIcons.lightbulb, primaryColor),
      ...
      child: Shimmer.fromColors(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(height: 20, width: 200, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(6))),
            ...
            ListView.builder(
              ...
              physics: const NeverScrollableScrollPhysics(),
              itemCount: 2,
              itemBuilder: (context, index) => Container(margin: const EdgeInsets.only(bottom: 12), height: 80, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12))),
            )
          ],
        ),
      ),
  ...
  Widget _buildSectionHeader(String title, IconData icon, Color color, {VoidCallback? onViewAll}) {
    return SliverToBoxAdapter(
      child: Padding(
        padding: const EdgeInsets.fromLTRB(20, 24, 16, 12), // Disesuaikan padding kanan
        child: Row(
          ...
          children: [
            ...
            if (onViewAll != null)
              TextButton(
                ...
                child: Text('Lihat Semua', style: TextStyle(fontSize: 13, color: color, fontWeight: FontWeight.w500)),
              ),
          ],
        ),
      ),
    ...
    return SliverToBoxAdapter(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Obx(() => Text( // Menggunakan controller untuk username dari UserService
                  'Hi, ${controller.userService.username.value}! 👋',
                  style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.grey[850]),
                )),
      ...
      child: Padding(
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                Obx(()=> Row( // Obx untuk statistik
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    _buildStatItem(icon: LucideIcons.activity, value: '${controller.averageScore.value.toStringAsFixed(1)}', label: 'Rata-rata', color: Colors.amber.shade700),
                    _buildStatItem(icon: LucideIcons.flame, value: '${controller.consecutiveDays.value}', label: 'Streak', color: primaryColor),
                    _buildStatItem(icon: LucideIcons.gem, value: '${controller.tokens.value}', label: 'Token', color: Colors.blue.shade600),
                  ],
                )),
                const SizedBox(height: 20),
                Obx(() => Column( // Obx untuk progress bar
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text('Progres Keseluruhan Kamu', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.grey[700])),
                            Text('${(controller.overallProgress.value * 100).toStringAsFixed(0)}%', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: primaryColor)),
                          ],
                        ),
                        const SizedBox(height: 8),
                        ClipRRect(
    ...
    return SliverPadding(
      ...
      sliver: SliverGrid(
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: MediaQuery.of(Get.context!).size.width > 450 ? 4 : 2, // Lebih responsif
          mainAxisSpacing: 16,
          crossAxisSpacing: 16,
          childAspectRatio: 0.6, // Kartu dibuat lebih persegi
        ),
        delegate: SliverChildListDelegate(
          controller.quickActionItems.map((item) => _buildQuickActionCard(
            icon: item['icon'] as IconData,
            title: item['title'] as String,
            color: controller.parseColor(item['colorHex'] as String),
            onTap: item['navigateTo'] != null 
                ? () => controller.navigateToLatihanPage() // Navigasi ke halaman Latihan
                : () => controller.navigateToPractice(item['practiceType'] as String),
          )).toList(),
        ),
      ),
    );
  ...
  Widget _buildLatestActivitiesList(ThemeData theme) {
    return Obx(() { // Obx untuk activities
      if (controller.activities.isEmpty) return const SliverToBoxAdapter(child: SizedBox.shrink());
      return SliverList(
        delegate: SliverChildBuilderDelegate(
          ...
          childCount: controller.activities.length > 3 ? 3 : controller.activities.length,
        ),
      );
    });
  }
  ...
  Widget _buildActivityItem(ActivityModel activity, ThemeData theme) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(activity.title, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14.5, color: Colors.grey[800]), maxLines: 1, overflow: TextOverflow.ellipsis,),
        subtitle: Text(activity.date, style: TextStyle(fontSize: 12, color: Colors.grey[600])),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text('${activity.score}', style: TextStyle(fontWeight: FontWeight.bold, color: color, fontSize: 17)),
            Text('Skor', style: TextStyle(fontSize: 11, color: Colors.grey[600])),
          ],
        ),
        onTap: () => Get.toNamed('/activity-detail', arguments: activity), // Pastikan route ini ada
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      ),
    );
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(LucideIcons.brainCircuit, color: Colors.amber.shade700, size: 26),
                    const SizedBox(width: 10),
                    Text('Saran Cerdas Untukmu', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.amber.shade800)),
                  ],
                ),
                const SizedBox(height: 12),
                Obx(()=> Text( // Obx untuk recommendationText
                  controller.recommendationText.value,
                  style: TextStyle(color: Colors.brown.shade700, fontSize: 14, height: 1.5),
                )),
                const SizedBox(height: 16),
                Obx(() => Wrap( // Obx untuk practiceTypes
                      spacing: 10,
                      runSpacing: 8,
                      children: controller.practiceTypes.map((type) => Chip(
                            avatar: Icon(LucideIcons.checkCircle2, color: primaryColor, size: 16),
                            label: Text(type.title, style: TextStyle(color: primaryColor, fontWeight: FontWeight.w500, fontSize: 13)),
